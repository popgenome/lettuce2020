...
read_gff($pos_file,\%gene) if($Posformat eq 'gff' || $pos_file =~ /.(gff|gff2|gff3)$/);
...
while (<IN>) {
	...
	foreach  my $gene (sort keys %$chr_pp) {
		...
		if ($Type eq "mrna") {
			...
			Display_seq(\$mrna);
			my $mark = "$gene  locus=$chr:$exon[0][0]:$exon[-1][1]:$strand";
			$output .= ">".$mark."\n".$mrna;
		}
...
sub read_gff{
	...
	while (<IN>) {
		...
		my $qname;
		if ($t[2] eq 'mRNA' ) {
			$qname = $1 if($t[8] =~ /GenePrediction\s+(\S+)/ || $t[8] =~ /ID=([^;\s]+)/ );
		}elsif ($t[2] eq 'CDS'){
			$qname = $1 if($t[8] =~ /GenePrediction\s+(\S+)/ || $t[8] =~ /Parent=([^;\s]+)/);
		}
		
		if ($t[2] eq 'match' || $t[2] eq 'HSP') {
			$qname = $1 if($t[8] =~ /Target\s"([^"]+)"/);
		}
